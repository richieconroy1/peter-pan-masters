...
export default function App() {
  ...
  const fetchLiveScores = async () => {
    try {
      ...
      const { leaderboard: lbData, scorecards: scData, hasLiveData } = json;
      ...
      if (scData?.players) {
        scData.players.forEach((p) => {
          ...
          (p.rounds||[]).forEach((round)=>{
            ...
            let roundDiffs=[],roundStrokes=0,roundComplete=true;
            holes.forEach((h)=>{
              if(h.strokes==null||h.par==null){roundComplete=false;return;}
              const diff=h.strokes-h.par;
              roundDiffs.push(diff);
              roundStrokes+=h.strokes;
              if(h.strokes===1&&h.par>=3)holeInOne++;
              if(diff<=-2)eagles++;
            ...
            for(let d of roundDiffs){if(d<=-1){streak++;if(streak>=3)streakFound=true;}else streak=0;}
          ...
          scorecardMap[p.id]={birdies,pars,bogeys,double_bogeys:doubles,triple_bogeys:tripleOrWorse,eagles,hole_in_one:holeInOne,birdie_streak_bonus:birdieStreakBonus,bogey_free_bonus:bogeyFreeBonus,all_rounds_under_70_bonus:allRoundsUnder70};
        });
      }
      ...
      const ticker2 = leaderboard
        ...
        .map((p)=>{
          ...
          const score=(rawScore!==null&&rawScore!==undefined&&!isNaN(Number(rawScore)))?Number(rawScore):null;
          return{pos:p.tied?`T${p.position}`:`${p.position||"–"}`,name:`${p.first_name||""} ${p.last_name||""}`.trim(),score};
        });
      ...
      leaderboard.forEach((p)=>{
        ...
        scores[fullName]={position,birdies:sc.birdies||0,eagles:sc.eagles||0,pars:sc.pars||0,bogeys:sc.bogeys||0,double_bogeys:sc.double_bogeys||0,triple_bogeys:sc.triple_bogeys||0,hole_in_one:sc.hole_in_one||0,birdie_streak_bonus:sc.birdie_streak_bonus||0,bogey_free_bonus:sc.bogey_free_bonus||0,all_rounds_under_70_bonus:sc.all_rounds_under_70_bonus||0};
      });
      setLiveData(scores);
        ...
        playersData.forEach((p)=>{
          ...
          (p.linescores||[]).forEach((round)=>{
            ...
            holes.forEach((h)=>{
              ...
              if(hScore==null||hPar==null){roundComplete=false;return;}
          ...
          if(completedRoundScores.length===4&&completedRoundScores.every(s=>s<70))allRoundsUnder70=1;
          espnScores[pName]={position,birdies,eagles,pars,bogeys,double_bogeys:doubles,triple_bogeys:0,hole_in_one:holeInOne,birdie_streak_bonus:birdieStreakBonus,bogey_free_bonus:bogeyFreeBonus,all_rounds_under_70_bonus:allRoundsUnder70};
        });
