...
export default function App() {
  ...
  const fetchLiveScores = async () => {
    try {
      ...
      const teeTimesData = lbData?.tee_times || [];

      // Before tournament starts — show tee times in ticker
      if (leaderboard.length === 0 && teeTimesData.length > 0) {
        const ticker = teeTimesData.slice(0, 30).map((t) => ({
          pos: t.tee_time ? new Date(t.tee_time).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true, timeZone: "America/New_York" }) : "–",
          name: `${t.player?.first_name || ""} ${t.player?.last_name || ""}`.trim(),
          score: null,
          isTeeTime: true,
        }));
      ...
      // Build ticker from leaderboard — guard against undefined score
